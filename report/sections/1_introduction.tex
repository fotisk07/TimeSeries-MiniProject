...
\begin{itemize}
    ...
    \item \textbf{Source Code \& Implementation:} We adapted the SoccerCPD code to be used as a python package. We also reproduced the core analysis by implementing it in Python, integrating an R-based backend for the Generalized Edge-Count test (g-segmentation) to improve detection accuracy. We also implemented a Python-based fallback for change-point detection to ensure robustness when the R-bridge encounters errors.
    \item \textbf{Experiments:} We verified the method on synthetic datasets, successfully detecting ground-truth change points. We also performed a sensitivity analysis using the provided source code.
    For real-world applications, we analyzed a Barcelona match, specifically detecting tactical phases and visualizing role allocations.
